...
namespace GoldenDates.WebService.Controllers
{
    ...
    public class ProductoController : Controller
    {
        ...
        //CONSULTAR
        [HttpGet]
        public JsonResult GetItem(int id_prod)
        {
            var item = new Producto();
            using (var bd = new bdgoldendatesEntities1())
            {
                item = bd.Productos.Where(w => w.id_prod == id_prod).FirstOrDefault();
            }
        ...
        //ELIMINAR
        [HttpPost]
        public JsonResult DeleteItem(int id_prod)
        {
            bool result = false;
            using (var bd = new bdgoldendatesEntities1())
            {
                try
                {
                    var item = bd.Productos.Where(w => w.id_prod == id_prod).FirstOrDefault();
